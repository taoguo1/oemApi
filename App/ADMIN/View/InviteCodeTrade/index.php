...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('InviteCodeTrade')?>" method="post">
        <div class="searchBar">
            <ul class="searchContent">
                <li><label>邀请码：</label> <input type="text" name="code" value="<?=\Core\Lib::request('code')?>" /></li>
                <li><label>卖方代理：</label> <input type="text" name="nickname" value="<?=\Core\Lib::request('nickname')?>" /></li>
                <li><label>买方代理：</label> <input type="text" name="after_name" value="<?=\Core\Lib::request('after_name')?>" /></li>
                <li><label>时间：</label>
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            ...
            <!--<li><a title="添加" target="dialog" rel="InviteCodeTradeEdit"  href="<?=\Core\Lib::getUrl('InviteCodeTrade','trade');?>" class="edit" width="450" height="250"><span>批量分配</span></a></li>-->
        </ul>
    </div>
    <table class="list" width="100%" layoutH="90">
        ...
        <tr>
            ...
            <th align="center" orderField="A.code_id" class="<?=($data['orderField']=='A.code_id')?$data['orderDirection']:'';?>">邀请码ID</th>
            <th align="center" orderField="A.code" class="<?=($data['orderField']=='A.code')?$data['orderDirection']:'';?>">二维码</th>
            <th align="center" orderField="A.code" class="<?=($data['orderField']=='A.code')?$data['orderDirection']:'';?>">邀请码</th>
            <th align="center" orderField="B.nickname" class="<?=($data['orderField']=='B.nickname')?$data['orderDirection']:'';?>">卖方</th>
            <th align="center" orderField="C.nickname" class="<?=($data['orderField']=='C.nickname')?$data['orderDirection']:'';?>">买方</th>
            <th align="center" orderField="A.trade_time" class="<?=($data['orderField']=='A.trade_time')?$data['orderDirection']:'';?>">交易时间</th>
        </tr>
        </thead>
        <tbody>
        <?php foreach ($data['list'] as $k=>$v){?>
            <tr target="id" rel="<?=$v['id']?>">
                ...
                <td align="center"><?=$v['code_id']?></td>
                <td align="center"><a title="扫一扫获取邀请码" href="<?=\Core\Lib::getUrl('InviteCode', 'createQr','id='.$v['code'].'&appid='.\Core\Lib::request('appid'));?>" target="dialog" width="400" height="400"><img  src="/Qr/createQr/<?php echo $v['code'].'?appid='.\Core\Lib::request('appid');?>" width="50" height="50"></a></td>                
                <td align="center"><?=$v['code']?></td>
                <td align="center"><?=$v['nickname']?></td>
                <td align="center"><?=$v['after_name']?></td>                
                <td align="center"><?= \Core\Lib::uDate('Y-m-d H:i:s x',$v['trade_time']);?></td>
            </tr>
        <?php }?>
        </tbody>
    </table>
