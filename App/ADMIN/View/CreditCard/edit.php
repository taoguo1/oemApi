<div class="pageContent">
	...
	<form method="post" action="<?=\Core\Lib::getUrl('CreditCard','edit','id='.$list['id'].'&act=edit');?>" class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone);">
		<div class="pageFormContent" layoutH="56">
            ...
            <div class="unit">
                <dl>
                    <dt>安全码：</dt>
                    <dd>
                        <input type="text" style="width:300px;" name="cvn" class="required" value="<?=\Core\Lib::aesDecrypt($list['cvn'])?>"  placeholder="" />
                    </dd>
                </dl>
            </div>
            <div class="unit">
                <dl>
                    <dt>有效期：</dt>
                    <dd>
                        <input type="text" name="expiry_date" dateFmt="MM/yy" readonly class="required date" value="<?=\substr_replace(\Core\Lib::aesDecrypt($list['expiry_date']), '/', 2, 0);?>" style="width: 300px"/>
                    </dd>
                </dl>
            </div>
            <!--<div class="unit">
                    ...
                    <dd>
                        <select name="bill_day" class="required">
                            ...
                            <?php for ($i=1;$i<=31;$i++)
                            {
                                $day = ($i<=9)?'0'.$i:$i;
                                ?>
                                <option  value="<?php echo $day;?>" <?php if($list['bill_day']==$day){echo 'selected';} ?>>每月<?php echo $day;?>日</option>
                                <?php
                            }
                            ?>
                        </select>
                    ...
                    <dd>
                        <select name="repayment_day" class="required">
                            <?php for ($i=1;$i<=31;$i++)
                            {
                                $day = ($i<=9)?'0'.$i:$i;
                                ?>
                                <option  value="<?php echo $day;?>" <?php if($list['repayment_day']==$day){echo 'selected';} ?>>每月<?php echo $day;?>日</option>
                                <?php
                            }
                            ?>
                        </select>
