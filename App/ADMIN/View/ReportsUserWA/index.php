...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('ReportsUserWA','index')?>" method="post">
        <div class="searchBar">
...
<div class="pageContent">
    ...
    <table class="list" width="100%" layoutH="85">
        ...
        <tbody>
            ...
            <tr target="id" rel="<?=$j?>">
                ...
                <td align="center"><?php
                         echo \Core\lib::formatMoney($day_data1[$j]['sum']);
                     ?></td>
                <?php
                ...
                <td align="center">
                    <?php
                         echo \Core\lib::formatMoney($day_data1[$j][$i]);
                     ?>
                </td>
                <?php
                }
            ...
            <tr target="id" rel="<?=$j?>">
                ...
                <td align="center"><?=\Core\lib::formatMoney($total1)?></td>
                <?php for($i = 0;$i <= 23;$i++){ ?>
                    <td align="center"><?=\Core\lib::formatMoney($sum1[$i])?></td>
                <?php }?>
            </tr>
    ...
    <div class="panelBar">
        <div class="pages">
            <span>合计：<?=\Core\lib::formatMoney($total1)?></span>
        </div>
    </div>
    <?php
        }else{
    ...
    <table class="list" width="100%" layoutH="85">
       ...
       <?php for($j = 1;$j <= 12;$j++){?>
            <tr target="id" rel="<?=$j?>">
                ...
                <td align="center"><?php echo \Core\lib::formatMoney($day_data[$j]['sum']); ?></td>
                <?php
                if(\Core\Lib::request('type') == 'week') {
                    for ($i = 0; $i <= 6; $i++) {
                        ...
                        <td align="center">
                            <?php
                            echo \Core\lib::formatMoney($day_data[$j][$i]);
                            ?>
                        </td>
                        <?php
                    }
        ...
        <tr target="id" rel="<?=$j?>">
            <td align="center"><strong>合计</strong></td>
            <td align="center"><?=\Core\lib::formatMoney($total)?></td>
            <?php if(\Core\Lib::request('type') == 'week'){for($i =0;$i <=6;$i++){ ?>
                <td align="center"><?=\Core\lib::formatMoney($weekMoney[$i]) ?></td>
            <?php }}else{for ($i = 1; $i <= 31; $i++) {?>
                <td align="center"><?=\Core\lib::formatMoney($sum[$i]) ?></td>
            <?php } } ?>
        </tr>
    ...
    <div class="panelBar">
        <div class="pages">
            <span>合计：<?=\Core\lib::formatMoney($total)?></span>
        </div>
    </div>
    <?php
    }
