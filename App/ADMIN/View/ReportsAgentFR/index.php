...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('ReportsAgentFR','index')?>" method="post">
        <div class="searchBar">
            <table class="searchContent">
                <tr>

                        <td class="agentfrtypeclick">
                    ...
                    <td class="tdcombox agentfrnummonth" <?php if(\Core\Lib::request('type')!='month'){?> style="display:none;"<?php }?> >
                    ...
                    <td class="tdcombox">
                        <select class="combox" name="in_type">
                            <option value="1" <?php if(\Core\Lib::request('in_type')=="1"){?> selected<?php }?> >自动</option>
                            <option value="2" <?php if(\Core\Lib::request('in_type')=="2"){?> selected<?php }?> >手动</option>
                        </select>
                    </td>
                    <td>
...
<div class="pageContent">
 ...
 <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
    <div id="agentfrmain" style="width: 90%;height:500px; margin-top:80px;"></div>
    <script type="text/javascript">
        // 基于准备好的dom，初始化echarts实例
        var myChart = echarts.init(document.getElementById('agentfrmain'));

        // 指定图表的配置项和数据
        var option = {
            title: {
                text: '合计：<?php echo $datasum;?>元',
				left:'110px;',
				textStyle: {
					fontSize: 16,
					fontWeight: 'bolder',
   ...
   <script type="text/javascript">
   $(".agentfrtypeclick").click(function(){
	   
	    var val=$(this).find('input:radio[name="type"]:checked').val();
		if(val=="month"){
			$(".agentfrnummonth").show();
		}else{
			$(".agentfrnummonth").hide();
		}
   })
   </script>
