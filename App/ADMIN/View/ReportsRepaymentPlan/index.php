...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('ReportsRepaymentPlan','index')?>" method="post">
        <div class="searchBar">
            <table class="searchContent">
                <tr>
                    ...
                    <?php }?>
                    <td>
                        <input size=25 name='keyword' type='text' alt='输入会员手机号或者姓名' value="<?=$phone?>">
                    </td>
                    <td style="white-space: inherit;">
                        <label>选择会员：</label>
                        <input type="text"  class="nickname" name="user.real_name" value="<?=\Core\Lib::request('user_real_name')?>" lookupGroup="user" readonly/>
                        <input type="hidden"  class="nickname" name="user.id" value="<?=\Core\Lib::request('user_id')?>" lookupGroup="user" />
                        <a class="btnLook" href="<?php echo \Core\Lib::getUrl('bill','users');?>" lookupGroup="user">选择会员</a>
                    </td>
                    <td>
                        <select class="combox" name="planlistStatus">
                            <option value="">选择状态</option>
                            <?php
                            foreach ($dic['planlistStatus'] as $key=>$val){
                                $selected = $key == $planlistStatus ? "selected" : "";
                    ...
                    <td>
                        <select class="combox" name="planType">

                            <option value="">选择类型</option>
                            <option <?= ($planType) ? "selected" : "" ?> value="1">还款</option>
                            <option <?= ($planType) ? "selected" : "" ?> value="2">消费</option>
