...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('ReportsYilianCV','index')?>" method="post">
        <div class="searchBar">
                    ...
                    <td>
                        <select class="combox" name="userid">
                            ...
                            foreach ($userlist as $key=>$val){
                                echo ' <option value="'.$val['id'].'">'.$val['real_name'].'</option>';
                            }
                            ?>
        ...
        <table class="list" width="100%" layoutH="90">
            ...
            <tr target="id" rel="<?=$j?>">
                ...
                if(\Core\Lib::request('type') == 'week'){for($i = 0;$i <= 6;$i++){ ?>
                    <td align="center"><?php echo array_sum($data['fday'][$i]);?></td>
                <?php
                    }
                }else{
                    for ($i = 1; $i <= 31; $i++) {?>
