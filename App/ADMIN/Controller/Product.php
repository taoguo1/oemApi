...
class Product extends Controller {
	...
	public function index($cid = 0) {
		...
		$productCategoryModel = new \App\ADMIN\Model\ProductCategory();
		$listOptionStr = $productCategoryModel->getOptionList ( $cid, '', $category_id );
		$start_date = Lib::request ( 'start_date' );
		$end_date = Lib::request ( 'end_date' );
		$recommend_level = Lib::request ( 'recommend_level' );
		if (! empty ( $category_id )) {
		    $ids = $productCategoryModel->getCatrgoryIds ( $category_id );
			$ids [] = $category_id;
			$category_id = $ids;
		}
		...
		($category_id) ? $condition ['AND'] ['category_id'] = $category_id : null;
	...
	public function add($cid = 0, $act = null) {
		...
		$this->assign ( "cid", $cid );
		$articleCategoryModel = new \App\ADMIN\Model\ProductCategory();
