...
class ReportsAgentWP extends Model
{
    ...
    public function getList($type=null, $condition = null,$year=null,$month=null)
    {
        ...
        $data=[];
        $data['day']['sumday']=0;
        $datalist=DBQ::getAll("bill(A)",['[>]agent(B)'=>["A.agent_id"=>"id"]],['A.poundage','A.create_time',"B.id","B.mobile"],$condition);
        if(empty($month)){
            for($j = 1;$j <= 12;$j++){
                if($type=="day"){
                        ...
                        if($i<=Lib::getMonthLastDay($year,$j)){
                            ...
                            foreach ($datalist as $k=>$v) {
                                if ($v['create_time'] >= $daystart && $v['create_time'] < $dayend) {
                                    $data['day'][$j][$i]=$data['day'][$j][$i]+$v['poundage'];
                                    $data['fday'][$i][$j]+=$v['poundage'];
                                }
                            ...
                            if($weeknum==$i){
                                ...
                                foreach ($datalist as $key=>$val) {
                                    if ($val['create_time'] >= $daystart && $val['create_time'] < $dayend) {
                                        $data['day'][$j][$i]=$data['day'][$j][$i]+$val['poundage'];
                                        $data['fday'][$i][$j]+=$val['poundage'];
                                    }
