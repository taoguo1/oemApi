...
class ProductCategory extends Model
{
    ...
    {
        $list = DBQ::getAll('product_category', '*', [
            'pid' => $id,
            'ORDER' => [
                'sort' => 'ASC'
            ]
    ...
    {
        $list = self::select('product_category', '*', [
            'pid' => $id,
            'ORDER' => [
                'sort' => 'ASC'
            ]
    ...
    public function del($id = 0)
    {
        if ($this->has('product_category', [
            ...
            if ($this->has('product', [
                ...
                Dwz::err('该分类下还有产品信息，请先删除资讯信息');
                return false;
            } else {
                return $this->delete('product_category', [
                    'id' => $id
                ]);
            }
        }
    ...
    {
        return $this->update('product_category', [
            'name' => Lib::post('name'),
            'alias' => Lib::post('alias'),
            'status' => Lib::post('status'),
            'icon' => Lib::post('icon'),
